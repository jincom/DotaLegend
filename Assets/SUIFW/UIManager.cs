...
//	    private static UIManager _Instance = null;
        ...
        public void Awake()
        {
            ...
            //_TraCanvasTransfrom = GameObject.Find("UIRoot").transform;
        ...
        {
            ShowUIForms(uiFormName, true, true);
        }

        public void ShowUIForms(string uiFormName, bool isLuaForm)
        {
            ShowUIForms(uiFormName, isLuaForm, true);
        }
        ...
        /// </summary>
        /// <param name="uiFormName">UI窗体预设的名称</param>
        public void ShowUIForms(string uiFormName, bool isLuaForm, bool loadFromAB)
        {
            ...
            if (baseUIForms == null)
            {
                LoadUIForm(uiFormName, isLuaForm, loadFromAB);
            }
            else
            {
                OnShowUIForms(baseUIForms, uiFormName);
            }


        }

        private void OnShowUIForms(BaseUIForm baseUIForms, string uiFormName)
        {
            //是否清空“栈集合”中得数据
        ...
        private void LoadUIForm(string uiFormName, bool isLuaForm, bool loadFromAB)
        {
            GameObject goUIAssetPrefabs = null;
            GameObject goCloneUIPrefabs = null;

            if (loadFromAB)
            {
                string assetName = uiFormName;
                string abName = uiFormName + AppConst.ExtName;

                ResManager.LoadPrefab(abName, assetName, delegate (UnityEngine.Object[] objs)
                {
                    if (objs.Length == 0)
                    {
                        Debug.LogError(assetName + "加载资源失败！");
                        return;
                    }

                    goUIAssetPrefabs = objs[0] as GameObject; //创建的UI克隆体预设

                    if (goUIAssetPrefabs == null)
                    {
                        Debug.LogError(assetName + "加载资源类型不匹配！");
                        return;
                    }

                    goCloneUIPrefabs = Instantiate(goUIAssetPrefabs) as GameObject;
                    goCloneUIPrefabs.name = assetName;

                    BaseUIForm baseUiForm = null;
                    if (isLuaForm)
                    {
                        baseUiForm = goCloneUIPrefabs.AddComponent<LuaUIForm>();
                    }
                    else
                    {
                    ...
                    Util.LogWarning("CreatePanel::>> " + name + " " + goCloneUIPrefabs);
                    ...
                    goCloneUIPrefabs.transform.localScale = Vector3.one;
                    goCloneUIPrefabs.transform.localPosition = Vector3.zero;
                    //设置隐藏
